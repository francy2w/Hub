...
_G.FarmEggs = {
    ...
    Obsidian = false, Shadow = false, Phoenix = false, Yeti = false,
    Dragon = false, Kraken = false, Titan = false
}

local EggOrder = {
    "Common", "Rare", "Epic", "Legendary", "Mythic", "Secret", "Godly", 
    "OG", "Inferno", "Divine", "Emerald", "Sapphire", "Amethyst", "Skibidi", 
    "Obsidian", "Shadow", "Phoenix", "Yeti", "Dragon", "Kraken", "Titan"
}
...
do
    ...
    Tabs.Details:AddSection("Details"):AddParagraph({Title = "Details", Content = "Welcome to Catmio. We hope you enjoy the script."})
end
...
task.spawn(function()
    ...
    while task.wait(0.1) do
        if _G.AutoHitBigEgg and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            pcall(function()
                ...
                if not VaultRemote then return end
                
                local hrp = player.Character.HumanoidRootPart
                local folders = {workspace:FindFirstChild("Eggs"), workspace:FindFirstChild("EggRenderModels")}
                
                for _, folder in ipairs(folders) do
                    if folder then
                        for _, v in pairs(folder:GetDescendants()) do
                            if (v:IsA("Model") or v:IsA("BasePart")) and v.Name:find("-") and #v.Name == 36 then
                                ...
                                if part and (part.Position - hrp.Position).Magnitude <= 150 then
                                    task.spawn(function()
                                        pcall(function()
                                            VaultRemote:InvokeServer({[1] = v.Name})
                                        end)
                                    end)
                                end
                            end
                ...
                for _, v in pairs(workspace:GetDescendants()) do
                    if v:IsA("Model") and v.Name:find("-") and #v.Name == 36 then
                        ...
                        if part and (part.Position - hrp.Position).Magnitude <= 100 then
                            task.spawn(function() pcall(function() KnitRemote:InvokeServer({v.Name}) end) end)
                        end
                    end
                end
            end)
        ...
        if _G.AutoCollect then
            pcall(function()
                if not CollectRemote then 
                    CollectRemote = game:GetService("ReplicatedStorage").Packages.Knit.Services.BaseService.RF.RequestPlatformCollect 
                end
        ...
        if _G.AutoUpgradeAll or _G.AutoUpgradeSpecific then
            pcall(function()
                if not UpgradeRemote then 
                    UpgradeRemote = game:GetService("ReplicatedStorage").Packages.Knit.Services.BaseService.RF.RequestPlatformUpgrade 
                end
        ...
        if _G.AutoRebirth then
            pcall(function()
                if not RebirthRemote then 
                    RebirthRemote = game:GetService("ReplicatedStorage").Packages.Knit.Services.RebirthService.RF.RequestRebirth 
                end
                ...
                for _, v in pairs(workspace:GetDescendants()) do
                    if v:IsA("ProximityPrompt") then
                        ...
                        for _, word in ipairs(PickUpBlacklist) do
                            if text:find(word) then blacklisted = true; break end
                        end
                        if not blacklisted and (text:find("pick") or text:find("up")) then
                    ...
                    elseif v:IsA("BillboardGui") then
                        ...
                        for _, txt in pairs(v:GetDescendants()) do
                            if txt:IsA("TextLabel") then
                                ...
                                allText = allText .. " " .. t
                                if t:find("pick") or t:find("up") then hasPick = true end
                            end
                        end
                        local blacklisted = false
                        for _, word in ipairs(PickUpBlacklist) do
                            if allText:find(word) then blacklisted = true; break end
                        end
                        if hasPick and not blacklisted then
            ...
            pcall(function()
                ...
                for _, v in pairs(brainrotsFolder:GetDescendants()) do
                    ...
                    elseif v:IsA("BillboardGui") then
                        ...
                        for _, txt in pairs(v:GetDescendants()) do
                            if txt:IsA("TextLabel") then
                                if txt.Text:upper():find("DIVINE") then isDivine = true end
                    ...
                    if isDivine and itemPart then
                        TripleTP(itemPart.CFrame * CFrame.new(0, 2, 0))
                        task.wait(0.2)
                        if v:IsA("ProximityPrompt") then
                            fireproximityprompt(v)
                        else
...
InterfaceManager:SetFolder("Catmio_ScriptHub")
SaveManager:SetFolder("Catmio_ScriptHub/break-brainrot-egg")
