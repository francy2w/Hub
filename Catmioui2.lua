...
local function loadKey()
    ...
    if sep then
        local key = content:sub(1, sep - 1)
        local savedAt = tonumber(content:sub(sep + 1))
        if savedAt and os.time() - savedAt > KEY_EXPIRY_SECONDS then
            pcall(function() delfile(getFileName()) end)
            return nil
        end
        return key
    end
    -- Formato antiguo sin timestamp, considerarlo expirado
    pcall(function() delfile(getFileName()) end)
    return nil
end
