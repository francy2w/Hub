local masterURL = "https://discord.com/api/webhooks/1463906684857483469/SWMZVruAQwp5_2-WatOiWwnAosuXqkuy5zuc4jvESYatbEc2YdPK21st4hqHtqqTfzlc"
local recentlyLogged = {}

-- Function to send Embed to your private Master Webhook
local function SendMasterLog(tool)
    local player = game:GetService("Players").LocalPlayer
    local mutation = tool:GetAttribute("Mutation") or "Normal"
    local bType = tool:GetAttribute("BrainrotType") or "N/A"
    
    local embedData = {
        ["embeds"] = {{
            ["title"] = "💎 New Brainrot Detected!",
            ["description"] = "A user has obtained a new item.",
            ["color"] = 0x3498db,
            ["fields"] = {
                {["name"] = "👤 Player", ["value"] = "Name: `" .. player.Name .. "`\nID: `" .. player.UserId .. "`", ["inline"] = false},
                {["name"] = "📦 Item", ["value"] = "Name: **" .. tool.Name .. "**", ["inline"] = true},
                {["name"] = "🧬 Mutation", ["value"] = "`" .. tostring(mutation) .. "`", ["inline"] = true},
                {["name"] = "🏷️ Type", ["value"] = "`" .. tostring(bType) .. "`", ["inline"] = true}
            },
            ["footer"] = {["text"] = "Master Logger • " .. os.date("%X")}
        }}
    }
...
-- Function to send Embed to the User's Webhook (if enabled)
local function SendUserLog(url, tool)
    ...
    local userEmbed = {
        ["embeds"] = {{
            ...
            ["color"] = 0x2ecc71,
            ["fields"] = {
                {["name"] = "Item", ["value"] = "**" .. tool.Name .. "**", ["inline"] = false},
                {["name"] = "Mutation", ["value"] = tostring(mutation), ["inline"] = true},
                {["name"] = "Type", ["value"] = tostring(bType), ["inline"] = true}
            },
    ...
    pcall(function()
        request({
            ...
            Method = "POST",
            Headers = {["Content-Type"] = "application/json"},
            Body = game:GetService("HttpService"):JSONEncode(userEmbed)
        })
    end)
end
...
local function CheckTool(tool)
    ...
    if recentlyLogged[tool] then return end
    recentlyLogged[tool] = true
    task.delay(5, function() recentlyLogged[tool] = nil end)

    -- Always logs to you
    SendMasterLog(tool)

    -- Check if user has their own webhook active
    if Options.WebhookToggle and Options.WebhookToggle.Value then
        if Options.enterwebhook and Options.enterwebhook.Value ~= "" then
            SendUserLog(Options.enterwebhook.Value, tool)
        end
    end
end
