--// Fluent Library Setup (Enlaces solicitados)
...
--// Variables de control de Autofarm
local AutoBuy = false
...
-- live counters para el status panel
...
--// FLUENT INTERFACE CREATION
local Section = Tabs.Main:AddSection("Autofarms")

local ToggleBuy = Tabs.Main:AddToggle("AutoBuy", {Title = "Auto Buy", Default = false})
ToggleBuy:OnChanged(function()
    AutoBuy = Options.AutoBuy.Value
    Fluent:Notify({
        Title = "Auto Buy",
        Content = AutoBuy and "Enabled" or "Disabled",
        Duration = 3,
    })
end)

local ToggleUpgrade = Tabs.Main:AddToggle("AutoUpgrade", {Title = "Auto Upgrade", Default = false})
ToggleUpgrade:OnChanged(function()
    AutoUpgrade = Options.AutoUpgrade.Value
    Fluent:Notify({
        Title = "Auto Upgrade",
        Content = AutoUpgrade and "Enabled" or "Disabled",
        Duration = 3,
    })
end)

local ToggleFruit = Tabs.Main:AddToggle("AutoFruit", {Title = "Auto Fruit", Default = false})
ToggleFruit:OnChanged(function()
    AutoFruit = Options.AutoFruit.Value
    Fluent:Notify({
        Title = "Auto Fruit",
        Content = AutoFruit and "Enabled" or "Disabled",
        Duration = 3,
    })
end)
...
ToggleRebirth:OnChanged(function()
    ...
    if AutoRebirth and not getRebirthRemote() then
        Fluent:Notify({
            Title = "Auto Rebirth",
    ...
    Fluent:Notify({
        Title = "Auto Rebirth",
        Content = AutoRebirth and "Enabled" or "Disabled",
        Duration = 3,
    })
end)

local ToggleEvolve = Tabs.Main:AddToggle("AutoEvolve", {Title = "Auto Evolve (x10 income)", Default = false})
ToggleEvolve:OnChanged(function()
    AutoEvolve = Options.AutoEvolve.Value
    if AutoEvolve and not getEvolveRemote() then
        Fluent:Notify({
            Title = "Auto Evolve",
            Content = "Evolve remote not found in your tycoon!",
            Duration = 5,
        })
        Options.AutoEvolve:SetValue(false)
        AutoEvolve = false
        return
    end
    Fluent:Notify({
        Title = "Auto Evolve",
        Content = AutoEvolve and "Enabled (evolves at full progress)" or "Disabled",
        Duration = 3,
    })
end)

local TogglePower = Tabs.Main:AddToggle("AutoPowerLevel", {Title = "Auto Power Level", Default = false})
TogglePower:OnChanged(function()
    AutoPowerLevel = Options.AutoPowerLevel.Value
    Fluent:Notify({
        Title = "Auto Power Level",
        Content = AutoPowerLevel and "Enabled" or "Disabled",
        Duration = 3,
    })
end)

local SectionActions = Tabs.Main:AddSection("Sewer Actions")
...
Tabs.Main:AddButton({
    Title = "Vine Harvest",
    Callback = function()
        Fluent:Notify({ Title = "Vine Harvest", Content = "Running...", Duration = 2 })
        task.spawn(function()
...
--// MANAGERS SETUP (Saves & Interfaces)
...
InterfaceManager:SetFolder("catmioScriptHub")
SaveManager:SetFolder("catmioScriptHub/sell-lemons")

InterfaceManager:BuildInterfaceSection(Tabs.Settings)
SaveManager:BuildConfigSection(Tabs.Settings)

--// LIVE STATUS PANEL (Native CoreGui Overlay)
task.spawn(function()
...
Fluent:Notify({
    Title = "catmio",
    Content = "The script has been loaded properly.",
    Duration = 8
})
